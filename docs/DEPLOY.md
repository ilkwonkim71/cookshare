...
브라우저 ──https──▶ Vercel (Next.js)
   └────https, fetch(NEXT_PUBLIC_API_URL)────▶ Railway (Express API) ──▶ SQLite (Volume /data)
```
...
- **SQLite + 단일 인스턴스**: 백엔드는 1개 인스턴스로만(파일 DB). 스케일아웃하려면 Postgres 전환 필요(리스크 R10).
